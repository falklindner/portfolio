...
def df_comdirect():
    ...
    cd_view["Kaufwert"] = pfview.loc[lastday:,("Altersvorsorge",symbol_slicer,"Transactions")].values.flatten().transpose() - pfview.loc[lastday:,("Altersvorsorge",symbol_slicer,"Fees")].values.flatten().transpose()
...
def df_portfolio():
    ...
    df["Kaufwert"] = pfview.loc[lastday:,(slice(None),sum_slicer,"Turnover")].values.flatten().transpose()
